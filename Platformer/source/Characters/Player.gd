...
func _on_EnemyDetector_area_entered(area: Area2D) -> void:
	_velocity = stomp_velocity(_velocity, stomp_impulse)
...
func _physics_process(delta: float) -> void:
	...
	_velocity = move_and_slide(_velocity, FLOOR_UP)
...
func stomp_velocity(linear_velocity: Vector2, impulse: float) -> Vector2:
